...
const searchClientByIdType = async (echisClientDoc) => {
  ...
  const res = await axiosInstance.get(`partners/registry/search/${idType}/${idNumber}`);
  console.log(`${res.status}: ${res.statusText}`);
  return res.data.clientExists && res.data.client.clientNumber;
};

const createClientInRegistry = async (client) => {
  const res = await axiosInstance.post(`partners/registry`, JSON.stringify(client));
  console.log(`${res.status}: ${res.statusText} - ${res.data.client.clientNumber}`);
...
const generateToken = async () => {
  ...
  const res = await axios.post('https://dhpidentitystagingapi.health.go.ke/connect/token', data, config);
  console.log(`${res.status}: ${res.statusText} `);
